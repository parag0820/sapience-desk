...
const ArticleDetail = () => {
  ...
  const goToFilterPage = (type, value, title, id = null) => {
    setShowExploreModal(false); // Close modal on navigate
  ...
  return (
    ...
    >
      <Container>
        {/* CENTERED AND WIDENED ARTICLE LAYOUT */}
        <Row className="gy-4 justify-content-center">
          <Col lg={10} md={12}>
            <div
              id="printable-article"
              className="bg-white p-4 p-md-5 rounded-4 shadow-sm main-article-container"
            >
              <div className="mb-3 d-flex justify-content-between align-items-center flex-wrap gap-3">
                ...
                <div className="d-flex align-items-center gap-2">
                  <button
                    ...
                    className="btn btn-sm btn-dark d-flex align-items-center gap-1 shadow-sm print-hide-btn rounded-pill px-3"
                  >
                    <CiSearch size={18} />{" "}
                    <span className="small fw-bold">Search & Explore</span>
                  </button>
                  <button
                    onClick={() => window.print()}
                    className="btn btn-sm btn-outline-secondary d-flex align-items-center gap-1 shadow-sm print-hide-btn rounded-pill px-3"
                  >
              ...
              <div className="d-flex align-items-center flex-wrap gap-3 text-muted small mb-3 pb-3 border-bottom border-light">
              ...
              <div className="article-content custom-rich-text">
      ...
      <Modal
        ...
        onHide={() => setShowExploreModal(false)}
        size="lg"
        centered
        scrollable
      >
        <Modal.Header closeButton className="border-0 pb-0">
          <Modal.Title className="fw-bold h4 px-2">
            Explore Articles
          </Modal.Title>
        </Modal.Header>
            ...
            <Col md={12}>
              <div className="bg-white p-4 rounded-4 shadow-sm border">
                ...
                <InputGroup className="bg-light rounded-pill border overflow-hidden px-2 py-1">
                  ...
                  <Button
                    variant="link"
                    className="text-dark border-0 p-0 ps-2 pe-2"
                  >
                    {isSearching ? (
                ...
                {searchResults.length > 0 && (
                  <ListGroup className="mt-3 shadow-sm border-0 rounded-3 overflow-hidden">
                    {searchResults.map((item, idx) => (
                      ...
                      >
                        <div
                          style={{ fontSize: "11px" }}
                          className="text-warning fw-bold text-uppercase mb-1"
                        >
                          {item.type}
                        </div>
                ...
                <ListGroup variant="flush">
                  {recentPosts.map((post) => (
                    <ListGroup.Item
                      ...
                      className="border-0 px-0 py-2 text-dark bg-transparent hover-link"
                    >
                      • {post.title}
                    </ListGroup.Item>
                  ))}
            ...
            <Col md={6}>
              <div className="bg-white p-4 rounded-4 shadow-sm border h-100">
                ...
                {categories.map((cat) => (
                  <div
                    key={cat._id}
                    className="py-2 hover-link text-dark"
                    onClick={() =>
                      goToFilterPage(
                        "category",
                        cat.categoryName,
                ...
                <h6 className="fw-bold mt-4 mb-3 text-uppercase text-muted small border-bottom pb-2">
                  Archives
                </h6>
                {archives.map((a) => (
                  <div
      ...
      <style>{`
        /* --- THE ULTIMATE RICH TEXT RESET --- */
        
        .main-article-container {
          width: 100%;
          max-width: 100%;
          overflow-x: hidden; /* Only hides horizontal overflow on the main card */
        }
        ...
        /* Target absolutely EVERY element generated by the rich text editor */
        .custom-rich-text * {
          /* 1. Destroy any hardcoded fixed widths (e.g. width: 1200px) from the editor */
          max-width: 100% !important; 
          box-sizing: border-box !important;
          
          /* 2. Destroy any 'nowrap' behavior that prevents normal dropping to the next line */
          white-space: pre-wrap !important; 
          
          /* 3. Force extremely long unbroken strings (URLs, non-breaking spaces) to break */
          overflow-wrap: break-word !important; 
          word-wrap: break-word !important; 
          word-break: normal !important; 
        }

        /* Maintain nice vertical spacing */
        .custom-rich-text p { margin-bottom: 1.2rem; }
        .custom-rich-text h1, .custom-rich-text h2, .custom-rich-text h3 { margin-top: 1.8rem; margin-bottom: 1rem; }

        /* Ensure all media perfectly fits the container without bleeding off */
        .article-content img, 
        ...
        /* * EXCEPTIONS: Data tables and code blocks should NOT wrap aggressively.
         * They should maintain their structure and offer a horizontal scrollbar instead.
         */
        .custom-rich-text table,
        .custom-rich-text pre {
          display: block !important;
          width: 100% !important;
          overflow-x: auto !important;
          white-space: nowrap !important; /* Re-allow nowrap ONLY here */
...
//       // Use Promise.allSettled so one failed API doesn't break the whole sidebar
//       const results = await Promise.allSettled([
//         apiCall(API_ENDPOINTS.CATEGORY.GET_ALL),
//         apiCall(API_ENDPOINTS.ARTICLE.GET_ALL),
//         apiCall(
...
//       // 2. Handle Recent Posts & Archives
//       if (results[1].status === "fulfilled") {
//         const artRes = results[1].value;
//         const allArts = artRes?.articles || artRes?.data || [];
//         setRecentPosts(allArts.slice(0, 5));
